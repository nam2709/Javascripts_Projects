...
#from PostManager.models import PostImage
version = (3, 0)
    ...
    def download_image(self,image_url,image_format,main_directory,dir_name,count,print_urls,socket_timeout,prefix,print_size,no_numbering,no_download,save_source,img_src,silent_mode,thumbnail_only,format,ignore_urls):
        ...
        try:
            ...
            try:
                ...
                full_path = path
                print(str("Path: %s" % full_path))

                self.SUCCESS_LIST.append({'url': image_url, 'full_path': path})
                print('Current Success list: %s' % str(self.SUCCESS_LIST))
                try:
                    output_file = open(path, 'wb')
                    output_file.write(data)
                    output_file.close()
    ...
    def _get_all_items(self,page,main_directory,dir_name,limit,arguments):
        ...
        while count < limit+1:
            ...
            if object == "no_links":
                ...
                '''
                current_url = object['image_link']
                print('current_url = %s Page: %s' % (current_url, page))
                if current_url == last_url:
                    count += 1
                    continue
                else:
                    last_url = current_url
                if current_url in self.EXCLUDE_URLS:
                    print('Existed... Wait for continue...')
                    time.sleep(1)
                    count += 1
                    continue
    ...
    def download(self,arguments):
        ...
        if __name__ != "__main__":
            ...
            # if the calling file contains params directly
                ...
                if not arguments["silent_mode"]:
                    if arguments['print_paths']:
                        print(paths.encode('raw_unicode_escape').decode('utf-8'))
                #print('All Paths: %s' % str(full_paths))
        ...
        # for input coming from CLI
            ...
            if not arguments["silent_mode"]:
                if arguments['print_paths']:
                    print(paths.encode('raw_unicode_escape').decode('utf-8'))
        return paths_agg, errors, self.SUCCESS_LIST

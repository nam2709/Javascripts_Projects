...
//       $.ajax({
//           url: DepreciationDepreciation_URL,
//           type: "POST",
//           async: false,
//           cache: false,
//           timeout: 30000,
  ...
  tUpdatePostApi(formId=null){
    // alert('POST')
      ...
      var idForm = formData.get('uuid')
        ...
        $.ajax({
            url: DepreciationDepreciation_URL + idForm + "/",
            // type: "PUT",
            type: "PATCH",
            async: false,
            cache: false,
...
//       // 
//       $.ajax({
//           url: DepreciationDepreciation_URL + uuid + "/",
//           type: "PATCH",
//           async: false,
//           cache: false,
//           timeout: 30000,
...
//   }
            ...
            confirm: {
                ...
                action: function() {
                    $(this).ready(function(){
                        ...
                        $.ajax({
                            url: DepreciationDepreciation_URL + uuid_go + "/",
                            type: "DELETE",
                            async: false,
                            cache: false,
                            timeout: 30000,
  ...
  tGetAllObjApi(page=null,search_data=null){
      ...
      this.callAjax =
      $.ajax({
          url: DepreciationDepreciation_URL+has_go_page,
          type: "GET",
          //async: false,
          cache: false,
          timeout: 30000,
        ...
        for (var thId = 1; thId < tableHeaders.length; thId++){
            ...
            if (this.hasOwnProperty(attr)){
                    ...
                    //       }
                        ...
                        if(attr=="aset_name"){
                            html +=`<td class="text-wrap" style="min-width:200px;font-weight:800" onclick="DepreciationDepreciationDetails('`+this["uuid"]+`','`+ this.aset +`')"><a style="text-decoration: none;">` + (this[attr]) + `</a></td>`;
                                continue;
                        }
                      ...
                      `<td>
                        <div class="btn-group mb-2 mr-2">
                            ...
                            <div class="dropdown-menu">
                                ...
                                <a class="dropdown-item d-flex align-items-center L-Affiliate-Tagged" onclick="DepreciationDepreciationEdit('`+this.uuid+`')">               
                                ...
                                <a class="dropdown-item d-flex align-items-center L-Affiliate-Tagged" onclick="DepreciationDepreciationOnDeleteEvent('`+this.uuid+`')">               
                                    <i title="Xóa" class="far fa-trash-alt" onclick="DepreciationDepreciationOnDeleteEvent" aria-hidden="true"></i><span class="sr-only">Xóa</span>&nbsp;                
                                    Xóa Dữ Liệu          
                                </a> 
                            </div>
                        </div>
...
function DepreciationDepreciationEdit(uuid){
...
function ShowOption(URL,formid){
    for (var i = 1; i < 2; i++) {
        var pagess = "?page="+ i;
        $.ajax({
        url: URL+pagess,
...
$(document).ready(function(){ 
    ShowOption(DepreciationAsset_URL,"#asetnameDepreciationDepreciationEditModalInputId")
    ShowOption(DepreciationPeriod_URL,"#preioddetailDepreciationDepreciationEditModalInputId")
    ShowOption(DepreciationType_URL,"#typedepreciationDepreciationDepreciationEditModalInputId")
    ShowOption(DepreciationAsset_URL,"#asetnameDepreciationDepreciationCreateModalInputId")
    ShowOption(AssetRevaluationAssetRevaluation_URL,"#assetrevaluationDepreciationDepreciationEditModalInputId")
    ShowOption(DepreciationPeriod_URL,"#preioddetailDepreciationDepreciationCreateModalInputId")
    ShowOption(DepreciationType_URL,"#typedepreciationDepreciationDepreciationCreateModalInputId")
    ShowOption(AssetRevaluationAssetRevaluation_URL,"#assetrevaluationDepreciationDepreciationCreateModalInputId")
...
function ChangeCurrency(Currency){
    $.ajax({
        url: 'https://v6.exchangerate-api.com/v6/545c751c13b7291b25ccbab4/latest/USD',
        type: "GET",
        success: function(data) {
          var conversion_rates = data.conversion_rates;
          var changecurrency = conversion_rates[Currency];

...
# Copy ignore file types
    rsync --update -razv \
            ...
            --exclude '*.css' \
            --exclude '*.js' \
            --exclude '*.html' $APISRC/$APPNAME/* $APISRC-bin/$APPNAME/
fi
...
# Update All file type
    rsync --update -razv $APISRC-bin/$APPNAME/* -e "ssh -p 49322" root@sks.com:$APISRC/$APPNAME/
else
# Update ignore file types
    ...
    --exclude 'Yolo' \
    --exclude 'ZaloAI' \
    --exclude 'AndroidBuild' \
    $APISRC-bin/$APPNAME/* -e "ssh -p 49322" root@sks.com:$APISRC/$APPNAME/
fi

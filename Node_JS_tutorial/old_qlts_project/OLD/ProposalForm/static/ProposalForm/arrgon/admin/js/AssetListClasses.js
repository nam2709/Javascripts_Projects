function getCookie(cname) {
  ...
  var AssetList_arr_action = [
                  // default action
                  {
                      "title": "Xem chi tiết",
                      "func": "AssetListDetails",
                      "icon": "far fa-eye",
                      "href": "#",
                      "isCheck": false,
  ...
  // Actions functions
  function AssetListDetails(uuid){
      $('#assetListDetailmodalsId').modal('toggle');
      var obj=new AssetList();
      obj.tGetObjApi(uuid);
      obj.callAjax.then(function(data) {
          new AssetList(data).tFillFormModal('Detail','assetListDetailModalsFormId');

      })
      // obj.tFillFormModal('Detail');
  }
  
  function AssetListEdit(uuid){
      $('#assetListEditmodalsId').modal('toggle');
      var obj=new AssetList();
      obj.tGetObjApi(uuid);
      obj.callAjax.then(function(data) {
          new AssetList(data).tFillFormModal('Edit','assetListEditModalsFormId');
  
      })
      // obj.tFillFormModal('Edit');
  }
  
  function AssetListOnDeleteEvent(uuid){
      ...
      if(search_data == null){
          ...
          AssetListOnDeleteWithDataSearchEvent(uuid);
      }
  
  }
  function AssetListOnDeleteWithDataSearchEvent(uuid){
      $.confirm({
      icon: 'fa fa-smile-o',
      title: 'XÓA!',
      content: 'Bạn có chắc muốn xóa ?',
  ...
  function AssetListViewDetail(selectionId){
  ...
  class AssetList {
    ...
    constructor(data = null) {
      if (data != null) {
        ...
        if (data.hasOwnProperty('uuid')) {
          this.uuid = data.uuid;
          this.editUrl = '/ProposalForm/AssetList/edit/' + this.uuid + '/';
          this.detailUrl = '/ProposalForm/AssetList/detail/' + this.uuid + '/';
        }
        else {
          // this.uuid = null;
        }
    ...
    tGetFormData(formId = null) {
      var formEle = $("#" + formId);
      if (formEle.length > 0) {
        var chEle = formEle.find("#nameAssetListInputId"); //Kiêrm tra giá trị của form id name
        if (chEle.length > 0) {
          this.name = chEle.val();
        }
        else {
        ...
        var chEle = formEle.find("#uuidAssetListInputId");
        if (chEle.length > 0) {
          this.uuid = chEle.val();
        }
        else {
        ...
        var chEle = formEle.find("#codeAssetListInputId");
        if (chEle.length > 0) {
          this.code = chEle.val();
        }
        else {
        ...
        var chEle = formEle.find("#created_atAssetListInputId");
        if (chEle.length > 0) {
        ...
        var chEle = formEle.find("#updated_atAssetListInputId");
        if (chEle.length > 0) {
        ...
        var chEle = formEle.find("#created_byAssetListInputId");
        if (chEle.length > 0) {
        ...
        var chEle = formEle.find("#updated_byAssetListInputId");
        if (chEle.length > 0) {
        ...
        var chEle = $("#codeAssetListInputId");
        if (chEle.length > 0) {
          this.code = chEle.val();
        }
        else {
        ...
        var chEle = $("#nameAssetListInputId");
        if (chEle.length > 0) {
          this.name = chEle.val();
        }
        else {
        ...
        var chEle = $("#uuidAssetListInputId");
        if (chEle.length > 0) {
          this.uuid = chEle.val();
        }
        else {
        ...
        var chEle = $("#created_atAssetListInputId");
        if (chEle.length > 0) {
          this.created_at = chEle.val();
        }
        else {
        ...
        var chEle = $("#updated_atAssetListInputId");
        if (chEle.length > 0) {
          this.updated_at = chEle.val();
        }
        else {
        ...
        var chEle = $("#created_byAssetListInputId");
        if (chEle.length > 0) {
          this.created_by = chEle.val();
        }
        else {
        ...
        var chEle = $("#updated_byAssetListInputId");
        if (chEle.length > 0) {
          this.updated_by = chEle.val();
        }
        else {
    ...
    tFillForm() {
      var self = this;
      try {
        var j_ele_name = $("#nameAssetListInputId");
      ...
      try {
        var j_ele_uuid = $("#uuidAssetListInputId");
      ...
      try {
        var j_ele_tndid = $("#codeAssetListInputId");
    ...
    tFillFormModal(modals_type,formId = null) {
      ...
      try {
        var j_ele_uuid = $("#uuidAssetList"+apart+"InputId");
      ...
      try {
        var j_ele_name = $("#nameAssetList"+apart+"InputId");
        if (j_ele_name.length > 0) {
          if (j_ele_name.attr('name') != 'uuid') {
      ...
      try {
        var j_ele_code = $("#codeAssetList"+apart+"InputId");
      ...
      try {
        var j_ele_updated_at = $("#updated_atAssetList"+apart+"InputId");
        if (j_ele_updated_at.length > 0) {
          var dateObj = new Date(Date.parse(self.updated_at));
          if (dateObj != "Invalid Date") {
            var newdate = moment(dateObj).format('DD/MM/YYYY');
      ...
      try {
        var j_ele_created_at = $("#created_atAssetList"+apart+"InputId");
        if (j_ele_created_at.length > 0) {
          var dateObj = new Date(Date.parse(self.created_at));
          if (dateObj != "Invalid Date") {
            var newdate = moment(dateObj).format('DD/MM/YYYY');
    ...
    tCreatePostApi() {
      ...
      $.ajax({
        url: "/ProposalForm/api/AssetList/",
        type: "POST",
        async: false,
        cache: false,
        timeout: 30000,
    ...
    tUpdatePostApi(formId = null) {
      ...
      var arr_table = [];
      if (formId == null) {
        formData = new FormData($('#assetListCreateFormId')[0]);
      ...
      var code = formData.get('code')
          ...
          $.ajax({
              ...
              success: function (data) {
                    ...
                    AssetListGetDataTable(AccountAccountpagination["current_page"]);
                    //$(location).prop('href', "/Account/Account/detail/" + self.uuid + "/");
                    $('.modal').modal('hide');
                    // self.tFillForm();
              },
  ...
  return self;
    ...
    tDeleteApi(uuid = null) {
      ...
      $.ajax({
        url: "/ProposalForm/api/AssetList/" + uuid_go + "/",
        type: "DELETE",
        async: false,
        cache: false,
        timeout: 30000,
    ...
    tGetAllObjApi(page = null, search_data = null) {
      ...
      this.callAjax =
        $.ajax({
              url: "/ProposalForm/api/AssetList/" + has_go_page,
          type: "GET",
          //async: false,
          cache: false,
          timeout: 30000,
          ...
          success: function (data) {
            ...
            if (data.hasOwnProperty('results')) {
              for (var j = 0; j < data.results.length; j++) {
                var tmp = new AssetList(data.results[j]);
                results.push(tmp);
              }
    ...
    tSearchAllObjApi(page = null, search_data = null, typeSearch) {
      ...
      if (typeSearch == "filter") {
        SEARCH_URL = AssetList_FILTER_URL;
  
        slugSearch = "&";
  
        if ($("#nameAssetListFilterSearchInputId").length > 0) {
          var value = $("#nameAssetListFilterSearchInputId").val();
          if (value != "" && value != null) {
        ...
        SEARCH_URL = AssetList_SEARCH_URL;
  
        slugSearch = "&";
        slugSearch += "search=" + $("#assetListQuickSearchInputId").val();
    ...
    tGetObjApi(uuid) {
      ...
      this.callAjax =
        $.ajax({
          url: "/ProposalForm/api/AssetList/" + uuid+ "/",
          type: "GET",
          async: false,
          cache: false,
          timeout: 30000,
      ...
      if (table.length > 0) {
        ...
        for (var thId = 1; thId < tableHeaders.length; thId++) {
          ...
          if (this.hasOwnProperty(attr)) {
            ...
            if (attr == "name") {
              html += `<td class="text-wrap" style="min-width:300px" onclick="AssetListDetails('` + this["uuid"] + `')"><a>` + (this[attr]) + `</a></td>`;
              continue;
            }
  ...
  // fill table function
  function AssetListGetDataTable(page=1,search_data=null){
      search_log["search_func"] = "AssetListGetDataTable";
      ...
      obj.callAjax.then(function(data) {
      $("#assetListTableBodyId").empty();
      var body = $("#assetListDataTableId");
      //if (body.length > 0){
      ...
      if (results.length > 0) {
                  ...
                  pagenation_ele.append(`<li class="page-item"><a class="page-link" onclick="AssetListGetDataTable(1)">Đầu</a></li>`);
                  if (pagination["has_prev"] == true) {
                      pagenation_ele.append('<li class="page-item"><a class="page-link" onclick="AssetListGetDataTable(' + (parseInt(pagination["current_page"]) - 1) + ')">' + (parseInt(pagination["current_page"]) - 1) + '</a></li>');
                  }
                  ...
                  pagenation_ele.append(`<li class="page-item"><a class="page-link" onclick="AssetListGetDataTable(`+last_page_order+`)">Cuối</a></li>`);
                  }
              }
  })
  }
  ...
  $(document).ready(function(){
      var IdTable ="assetListDataTableId";
      var checker = $("#" +IdTable );
      if (checker.length > 0){
  ...
  $(document).ready(function(){
      var selectionId ="assetListDetailModalsFormId";
      AssetListViewDetail(selectionId)
  })
  
  // [Save Create] Clicked Handle function
  $(document).ready(function(){
    $("#assetListCreateModalBtnId").click(function(){
        var validate_obj = new InputValidation('assetListCreateModalsFormId');
        ...
        obj.tCreateNewPostFormApi('assetListCreateModalsFormId');
    })
  });
  
  // [SAVE UPDATE]
  $(document).ready(function(){
    $("#assetListUpdateModalBtnId").click(function(){
        obj = new AssetList();
        obj.tUpdatePostApi('assetListEditModalsFormId');
    })
  });
  ...
  // Export excel
  $(document).ready(function() {
    $('#assetListExportExcelBtnId').click(function() {
        // Define the columns to export
        var columnsToExport = [0, 1, 2, 3, 4, 5, 6]; // columns 1, 2, and 4 (zero-indexed)
        var theadToExport = '';
        $('#assetListDataTableId thead').each(function() {
        var row = '';
        ...
        var tbodyToExport = '';
        $('#assetListDataTableId tbody tr').each(function() {
        var row = '';
  ...
  function AssetListSearchData(page=1,search_type,search_data=null){
    search_log["search_func"] = "AssetListSearchData";
    ...
    search_log["search_data"] = search_data;
        ...
        if (results.length > 0) {
                pagenation_ele.append(`<li class="page-item"><a class="page-link" onclick="AssetListSearchData(1,'`+search_type+`'`+ `,'`+search_data+`'`+ `)">Đầu</a></li>`);
  
                if (pagination["has_prev"] == true) {
                    pagenation_ele.append(`<li class="page-item"><a class="page-link" onclick="AssetListSearchData(` + (parseInt(pagination["current_page"]) - 1) +`,'`+search_type+`'`+ `,'`+search_data+`'`+ `)">` + (parseInt(pagination["current_page"]) - 1) + `</a></li>`);
                }
                pagenation_ele.append('<li class="page-item active"><a class="page-link" >' + (parseInt(pagination["current_page"])) + '</a></li>');
                if (pagination["has_next"] == true) {
                    pagenation_ele.append(`<li class="page-item"><a class="page-link" onclick="AssetListSearchData(` + (parseInt(pagination["current_page"]) + 1) +`,'`+search_type+`'`+ `,'`+search_data+`'`+ `)">` + (parseInt(pagination["current_page"]) + 1) + `</a></li>`);
                ...
                pagenation_ele.append(`<li class="page-item"><a class="page-link" onclick="AssetListSearchData(`+last_page_order+`,'`+search_type+`'`+ `,'`+search_data+`'`+ `)">Cuối</a></li>`);
                }
            }
            if (search_type == "quick") {
                var crr_txt = $("#assetListQuickSearchInputId").val();
                highlight(crr_txt,"#assetListTableBodyId");
            }
    })
  ...
  $(document).ready(function(){
    $("#assetListQuickSearchInputId").on('keyup', function(e) {
        if (e.key === 'Enter' || e.keyCode === 13) {
            ...
            AssetListSearchData(AccountAccountpagination["current_page"],"quick");
        }
    })
    $("#assetListQuickSearchBtnId").click(function(){
        AccountAccountpagination={
            current_page:1,
            total:0,
            has_next:false,
    ...
    $("#assetListSearchBtnId").click(function(){
        AccountAccountpagination={
            current_page:1,
            total:0,
            has_next:false,
        ...
        AssetListSearchData(AccountAccountpagination["current_page"],"filter");
    })
  });
  ...
  function CheckUnique(code){
    ...
    $.ajax({
      url: AssetList_SEARCH_URL + slugSearch,
      dataType: 'JSON',
      type: 'GET',
      success: function(data){
        if (data.results.length > 0){
          toastr.warning('Mã danh sách đã tồn tại');
        }
          }
      });
  }
